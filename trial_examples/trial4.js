...
// more generlized approach for the notmalization and denomalization
// open: (open - lowest ) / (highest - lowest)

// first a function to find the lowest and highest number 
let LowestValue = 0;
let newArrayLow = [];


for (const key in rawData) {
    const LowNumbers = rawData[key].low
    newArrayLow.push(LowNumbers.toString());
}

// finding the maximum and minimum 
LowestValue = Math.min(...newArrayLow)


function normalization(step, Low){
    return {
        open: step.open / Low,
        high: step.high / Low,
        low: step.low / Low ,
        close: step.close / Low,       
    }
}

function denormalization(step, Low){
    return {
        open: step.open * Low,
        high: step.high * Low,
        low: step.low * Low ,
...
const Value =  normalization(rawData[0], LowestValue);
console.log(denormalization(Value, LowestValue))
